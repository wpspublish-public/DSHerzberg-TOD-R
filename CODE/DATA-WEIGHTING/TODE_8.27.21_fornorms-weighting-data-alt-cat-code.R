...
write_csv(unweighted_output,
          here(
            str_c(
              output_file_path,
              file_name,
              "unweighted-data-for-analysis-alt.csv"
            )
          ),
          na = "")
...
write_csv(weighted_output,
          here(
            str_c(
              output_file_path,
              file_name,
              "weighted-data-for-analysis-alt.csv"
            )
          ),
          na = "")
...
write_csv(demo_weight_by_crossing_input,
          here(
            str_c(
              output_file_path,
              file_name,
              "weights-per-demo-crossing-input-alt.csv"
            )
          ),
          na = "")
...
write_csv(demo_weight_by_crossing_all,
          here(
            str_c(
              output_file_path,
              file_name,
              "weights-per-demo-crossing-all-alt.csv"
            )
          ),
          na = "")
...
knitr::kable(
  weighted_output %>%
    ...
    select(-noweight, -(sege_sum:GradeSemester), -(i01_w:!!sym(
...
knitr::kable(
  tail(input_demo_wts) %>%
    select(-noweight, -(sege_sum:GradeSemester), -(i01:!!sym(
      str_c("i", last_item)
    )),-ratio),
  digits = 2,
