...
# three file names.
scale_file_name <- map_chr(
  form, 
  ~ 
    str_c("scale_lookup_", .x)
  )
...
perc_lookup <- suppressMessages(
  read_csv(
    here(
      "INPUT-FILES/OES-INPUT-TABLES/DP4-EXAMPLE/Percentile-Lookup-SS.csv"
      )))

# read in age labels for OES output lookup table
age_labels <- suppressMessages(
  read_csv(
    here(
      "INPUT-FILES/OES-INPUT-TABLES/DP4-EXAMPLE/OES-age-labels.csv"
      ))) %>%
...
scale_readin <- function(x) {
  ...
  here(
    str_c("INPUT-FILES/OES-INPUT-TABLES/DP4-EXAMPLE/", x, ".xlsx")) %>%
