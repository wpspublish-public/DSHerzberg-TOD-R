...
tests <- c("ernle", "esege", "lske", "lswe", "rhye", "spwe")
index_composites <- c("eddiw", "eddiq", "elp", "ersw", "ersq", "eswa", "ephk", "ebrs", "epha")
...
all_lookups <- bind_rows(age_lookups,
  ...
  mutate(
    equiv = case_when(
      ...
      norm_group == "age" & test == "ernle"  & between(raw, 0, 32) ~ "< start age",
      norm_group == "age" & test == "ernle"  & between(raw, 33, 40) ~ "5:0 to 5:3",
      norm_group == "age" & test == "ernle"  & between(raw, 41, 44) ~ "5:4 to 5:7",
      norm_group == "age" & test == "ernle"  & between(raw, 45, 50) ~ "5:8 to 5:11",
      norm_group == "age" & test == "ernle"  & between(raw, 51, 56) ~ "6:0 to 6:5",
      norm_group == "age" & test == "ernle"  & between(raw, 57, 65) ~ "6:6 to 6:11",
      norm_group == "age" & test == "ernle"  & between(raw, 66, 73) ~ "7:0 to 7:5",
      norm_group == "age" & test == "ernle"  & between(raw, 74, 79) ~ "7:6 to 7:11",
      norm_group == "age" & test == "ernle"  & between(raw, 80, 88) ~ "8:0 to 8:5",
      norm_group == "age" & test == "ernle"  & between(raw, 89, 99) ~ "8:6 to 9:3",
      norm_group == "age" & test == "ernle"  & between(raw, 100, 120) ~ "> stop age",
      ...
      norm_group == "grade" & test == "ernle"  & between(raw, 0, 35) ~ "< start grade",
      norm_group == "grade" & test == "ernle"  & between(raw, 36, 46) ~ "K-Fall",
      norm_group == "grade" & test == "ernle"  & between(raw, 47, 55) ~ "K-Spring",
      norm_group == "grade" & test == "ernle"  & between(raw, 56, 64) ~ "1-Fall",
      norm_group == "grade" & test == "ernle"  & between(raw, 65, 71) ~ "1-Spring",
      norm_group == "grade" & test == "ernle"  & between(raw, 72, 78) ~ "2-Fall",
      norm_group == "grade" & test == "ernle"  & between(raw, 79, 88) ~ "2-Spring",
      norm_group == "grade" & test == "ernle"  & between(raw, 89, 120) ~ "> stop grade",
    ...
    risk = case_when(
      ...
      test %in% c("eddiq", "eddiw") & between(ss, 90, 109) ~ "Low to Moderate Probability of Dyslexia", 
